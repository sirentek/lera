...
def _housekeeping_insight() -> None:
    """Lera: same piggy-back schedule as the curator, independently gated by
    insight.interval_hours. Produces the weekly digest + proactive suggestions
    the holographic memory provider injects at turn start."""
    from agent.insight import maybe_run_insight
    maybe_run_insight(idle_for_seconds=float("inf"), on_summary=lambda msg: logger.info("insight: %s", msg))
...
def _start_gateway_housekeeping(
    ...
    chores += [
        ...
        (60, "Insight tick", _housekeeping_insight),
