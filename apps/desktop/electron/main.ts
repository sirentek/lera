...
// Lera fork: upstream base-version update check (drives the "update available" hint).
import { checkLeraBaseVersion } from './lera-base-version'
...
// The Windows dev launcher runs a branded copy named Lera.exe. Electron treats
// any executable not named electron.exe as packaged, even while Vite is serving
// the renderer. DEV_SERVER is therefore the authoritative development signal, so
// it must veto the packaged detection even when HERMES_DESKTOP_IS_PACKAGED is set.
const IS_PACKAGED = (app.isPackaged || Boolean(process.env.HERMES_DESKTOP_IS_PACKAGED)) && !DEV_SERVER
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    show: false,
    // Lera: the frameless holo window is transparent off macOS, so the root
...
// Frameless holo-window controls. Resolve the sender's window so secondary
// windows cannot accidentally steer the main window.
...
function startManualResize(win, direction) {
  if (manualResizeTimer || win.isMaximized()) return

  const startCursor = screen.getCursorScreenPoint()
  const start = win.getBounds()
  const [minimumWidth, minimumHeight] = win.getMinimumSize?.() ?? [WINDOW_MIN_WIDTH, WINDOW_MIN_HEIGHT]
  const minWidth = minimumWidth || WINDOW_MIN_WIDTH
  const minHeight = minimumHeight || WINDOW_MIN_HEIGHT
  ...
  const apply = () => {
    ...
    if (width < minWidth) {
      if (direction.includes('left')) x = start.x + start.width - minWidth
      width = minWidth
    }
    if (height < minHeight) {
      if (direction.includes('top')) y = start.y + start.height - minHeight
      height = minHeight
    }
