...
test('appIconCandidates keeps the documented precedence ladder', () => {
  ...
  // Windows prepends the full-bleed .ico rungs ahead of the PNG ladder. LERA
  // FORK: the app tree's own assets/icon.ico outranks resources/ — in dev
  // `process.resourcesPath` is Electron's own resources and has no icon.ico.
  ...
  assert.equal(win.filter(c => c.endsWith('.ico')).length, 3)
  assert.equal(
    win[0],
    path.join('C:\\app', 'assets', 'icon.ico'),
    "the app tree's assets/icon.ico is the highest-precedence Windows rung"
  )
  assert.equal(win[1], path.join('C:\\app\\unpacked', 'assets', 'icon.ico'))
  assert.equal(win[2], path.join('C:\\resources', 'icon.ico'))
  assert.equal(
  ...
  // No resourcesPath (dev): the bogus cwd-relative rung is filtered out rather
  // than joined from a bare ''.
  const devWin = appIconCandidates({
    isWindows: true,
    appRoot: 'C:\\app',
    unpackedPathFor: p => `${p}\\unpacked`
  })

  assert.equal(devWin.filter(c => c.endsWith('.ico')).length, 2)
})
