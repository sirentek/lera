...
export function useStatusbarItems({
  ...
  const clientVersionItem = useMemo<StatusbarItem>(() => {
    ...
    // Lera: the fork also tracks the upstream hermes version it is based on, so
    // fall back to that when the desktop build has no appVersion of its own.
    ...
    // Lera: lead the tooltip with the base-version hint (the fork's own
    // "update available" signal), then upstream's client-version wording.
    const tooltip = [
    ...
    return {
      className: cn(
        status.hasUpdate && 'text-primary hover:text-primary',
