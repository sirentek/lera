...
def test_usage_ticker_emits_wrapped_usage_payload(monkeypatch):
    ...
    monkeypatch.setattr(server, "_get_usage", lambda agent, history=None: dict(snapshot))
...
def test_usage_ticker_stop_join_prevents_late_ticks(monkeypatch):
    ...
    def _blocking_get_usage(agent, history=None):
        calls["n"] += 1
        if calls["n"] == 1:
...
def test_run_prompt_submit_never_ticks_after_message_complete(monkeypatch):
    ...
    def _moving_usage(agent, history=None):
        counter["n"] += 1
...
def test_prompt_submit_empty_truncation_allowed_with_confirm(monkeypatch):
    ...
    try:
        monkeypatch.setattr(server.threading, "Thread", _ImmediateThread)
        monkeypatch.setattr(server, "_get_usage", lambda _a, history=None: {})
...
def _configure_immediate_prompt_run(
    ...
    monkeypatch.setattr(server, "_get_usage", lambda _agent, history=None: {})
    monkeypatch.setattr(
