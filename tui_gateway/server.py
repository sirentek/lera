...
def _get_usage(agent, history: list[dict] | None = None) -> dict:
    ...
    if agent is not None and history is not None and "context_percent" not in usage:
        # Lera: external context engines may not expose last_prompt_tokens. Estimate
        # the next request from the live prompt + history rather than falling back to
        # cumulative session_total_tokens, which is not window usage.
        try:
