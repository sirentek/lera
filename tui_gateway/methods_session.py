...
def _(rid, params: dict) -> dict:
    ...
    if agent is not None:
        # Lera: history-based context estimation — pass the live transcript so
        # the usage snapshot can estimate context load, not just raw call totals.
        with session["history_lock"]:
            history = list(session.get("history", []))
        usage: dict = _get_usage(agent, history=history)
    else:
        # No local agent: fall back to upstream's snapshot, which surfaces the
        # compute-host usage mirror for remote sessions.
