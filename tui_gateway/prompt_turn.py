...
def _complete_turn_payload(session: dict, st: _TurnRun, status_note: str | None, cols: int):
    ...
    # Lera: hand the live transcript to the usage snapshot so context load can be
    # estimated when the provider reports no last_prompt_tokens.
    with session["history_lock"]:
        usage_history = list(session.get("history", []))
    payload = {"text": raw, "usage": _get_usage(agent, history=usage_history), "status": status}
    if last_reasoning:
